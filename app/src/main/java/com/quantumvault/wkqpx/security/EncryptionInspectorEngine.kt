...
object EncryptionInspectorEngine {
    ...
    suspend fun analyzeBackupFile(context: Context, uri: Uri): BackupAnalysisResult = withContext(Dispatchers.IO) {
        ...
        val magicV1 = "VLT_BCK1".toByteArray(Charsets.UTF_8)
        val magicVaultBck = "VAULTBCK".toByteArray(Charsets.UTF_8)
        ...
        try {
            ...
            context.contentResolver.openInputStream(uri)?.use { inputStream ->
                ...
                if (headerBuf.contentEquals(magicV3)) {
                ...
                } else if (headerBuf.contentEquals(magicV1) || headerBuf.contentEquals(magicVaultBck)) {
                    // QUANTUM VAULT V1 (LEGACY PBKDF2 + AES-256-GCM)
                    formatName = "Quantum Vault V1 Legacy Archive"
                    magicHeaderStr = "${String(headerBuf, Charsets.UTF_8)} [${bytesToHex(headerBuf)}]"
                    isRecognizedEncrypted = true
                    securityScore = 4
                    securityLevelTitle = "LEVEL 4/5 — SECURE (LEGACY ARCHIVE)"
                    integrityVerdict = "VALID LEGACY ARCHIVE"

                    kdfSuite = "PBKDF2-HMAC-SHA256 (10,000 Iterations)"
                    kdfParams.add("Algorithm" to "PBKDF2WithHmacSHA256")
                    kdfParams.add("Iterations" to "10,000 rounds")
                    kdfParams.add("Salt" to "128-bit CSPRNG")

                    cipherSuite = "Streaming AES-256-GCM"
                    cipherParams.add("Cipher Mode" to "AES-256-GCM")
                    cipherParams.add("Key Size" to "256-bit")
                    cipherParams.add("Authentication Tag" to "128-bit GCM Tag")
                    ...
                    recommendations.add("Legacy backup archive is fully supported for restoration.")
                    recommendations.add("After restoration, create a new V3 backup for Argon2id protection.")
                    rawHeaderHexDump = buildSanitizedHeaderHexDump(rawSampleBytes)
